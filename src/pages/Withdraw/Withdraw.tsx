    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-16 pb-20 md:pb-4">
        <div className="max-w-lg mx-auto px-4 py-8">
          ...
          <p className="text-sm text-muted-foreground mb-6">Saldo disponível: <span className="text-neon-gold font-bold">R$ {balance.toFixed(2)}</span></p>

          <div className="card-casino rounded-2xl border border-border overflow-hidden">
            {step === "form" && (
              <div className="p-6 space-y-5">
                <div>
                  <label className="text-sm font-bold mb-2 block">Valor do Saque</label>
                  <Input type="number" placeholder={`Mínimo R$${MIN_WITHDRAW_AMOUNT}`} className="bg-secondary border-border text-lg font-bold" value={amount} onChange={(e) => setAmount(Number(e.target.value) || "")} />
                  <div className="flex gap-2 mt-2">
                    {QUICK_WITHDRAW_VALUES.map((v) => (
                      <button key={v} onClick={() => setAmount(Math.min(v, balance))} className="flex-1 py-1.5 text-xs font-bold bg-secondary border border-border rounded-lg hover:border-primary/50 transition-colors">
                        R${v}
                      </button>
                    ))}
                    <button onClick={() => setAmount(Math.floor(balance))} className="flex-1 py-1.5 text-xs font-bold bg-secondary border border-border rounded-lg hover:border-primary/50 transition-colors">
                      Tudo
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <label className="text-sm font-bold mb-2 block">Tipo de Chave PIX</label>
                  <div className="grid grid-cols-4 gap-1 mb-3">
                    {(["cpf", "email", "phone", "random"] as PixKeyType[]).map((t) => (
                      <button key={t} onClick={() => setPixType(t)} className={`py-2 text-xs font-bold rounded-lg border transition-colors ${pixType === t ? "bg-primary border-primary text-white" : "bg-secondary border-border"}`}>
                        {t === "cpf" ? "CPF" : t === "email" ? "Email" : t === "phone" ? "Telefone" : "Aleatória"}
                      </button>
                    ))}
                  </div>
                  <Input
                    placeholder={pixType === "cpf" ? "000.000.000-00" : pixType === "email" ? "seu@email.com" : pixType === "phone" ? "(11) 99999-9999" : "Chave aleatória UUID"}
                ...
                <div className="p-3 bg-secondary/50 rounded-xl border border-border text-xs text-muted-foreground">
                  Saques são processados em até 24 horas úteis. Valor mínimo R${MIN_WITHDRAW_AMOUNT}. Taxa: grátis via PIX.
                </div>
            ...
            {step === "processing" && (
              <div className="p-12 text-center">
                ...
                <p className="text-sm text-muted-foreground flex items-center justify-center gap-1"><Clock className="w-3 h-3" /> Aguarde</p>
              </div>
            )}

            {step === "done" && (
              <div className="p-12 text-center">
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="w-16 h-16 bg-primary/20 rounded-full flex items-center justify-center mx-auto mb-4 glow-purple">
                  <CheckCircle2 className="w-8 h-8 text-primary" />
                </motion.div>
                <p className="text-xl font-black mb-1">Solicitação Enviada!</p>
                <p className="text-sm text-muted-foreground mb-1">Seu saque de <span className="text-neon-gold font-bold">R$ {Number(amount).toFixed(2)}</span> está sendo processado.</p>
