...
import { casinoGames, tigreGames } from "./games/Cassino/data";
import CrashGame from "./games/Crash/Crash";
...
const sponsorCards = [
  {
    ...
    image:
      "https://images.unsplash.com/photo-1575361204480-aadea25e6e68?w=1200&h=600&fit=crop&q=80",
  },
  {
    title: "Parceiro Oficial",
    subtitle: "Junte-se à maior plataforma da América Latina",
    image:
      "https://images.unsplash.com/photo-1488426862026-3ee34a7d66df?w=1200&h=600&fit=crop&q=80",
  },
  {
    title: "Neymar Jr.",
    subtitle: "Parceiro Oficial",
    image:
      "https://images.unsplash.com/photo-1522778119026-d647f0596c20?w=1200&h=600&fit=crop&q=80",
  },
  {
    title: "Atlético Goianiense",
    subtitle: "Patrocinador Máster",
    image:
      "https://images.unsplash.com/photo-1560272564-c83b66b1ad12?w=1200&h=600&fit=crop&q=80",
  },
];

export default function HomePage() {
  const { user, isLoggedIn, isAdmin, openAuth, logout, initializing } =
    useAuth();
  const { balance } = useBalance();
  const location = useLocation();
  const navigate = useNavigate();
  const [showCasinoGames, setShowCasinoGames] = useState(
    location.pathname === "/cassino",
  );
  ...
  return (
    <div className="min-h-screen bg-[#071423] text-white">
        ...
        <div className="flex h-full w-full items-center justify-between px-4 md:px-6">
          <div className="flex items-center gap-4">
            ...
            <nav className="ml-8 hidden items-center gap-8 text-xs font-bold uppercase tracking-wide text-slate-300 md:flex">
              ...
              <a className="hover:text-white" href="#esportes">
                Esportes
              </a>
              <a
          ...
          <div className="flex items-center gap-3">
            {!initializing && isLoggedIn ? (
              <>
                <Link
                  to="/deposit"
                  className="hidden items-center gap-2 rounded-md border border-[#2a3e58] bg-[#0d2239] px-3 py-2 text-sm font-bold text-slate-100 md:flex"
                >
                ...
                <Link
                  to="/dashboard"
                  className="flex items-center gap-2 rounded-md border border-[#2a3e58] bg-[#0d2239] px-3 py-2 text-sm font-bold text-slate-100"
                >
              ...
              <>
                ...
                <Button
                  className="border-0 bg-[#f02254] text-sm font-black text-white hover:bg-[#ff2a60]"
      ...
      <div className="flex w-full min-h-[calc(100vh-4rem)]">
        <aside className="hidden w-[230px] shrink-0 border-r border-[#1e3149] bg-[#07172a] lg:block min-h-[calc(100vh-4rem)]">
          <div className="p-5">
            <p className="mb-5 text-xs font-black uppercase text-slate-300">
              Originais da NeonBet
            </p>
              ...
              {sideGames.map((item) => {
                if (item.label === "Crash") {
                  return (
                    <button
                      key={item.label}
                      className="flex w-full items-center gap-3 rounded-md px-2 py-2 text-left text-[22px] font-semibold text-slate-300 transition hover:bg-[#0d2239] hover:text-white"
                      onClick={() => {
                        setShowCasinoGames(false);
                        setShowCrash(true);
                      }}
            ...
            <div className="mt-7 border-t border-[#1e3149] pt-6">
              <p className="mb-4 text-xs font-black uppercase text-slate-300">
                Cassino
              </p>
              <div className="space-y-1">
                <button
                  ...
                  className="block w-full rounded-md px-2 py-2 text-left text-base font-semibold text-slate-300 transition hover:bg-[#0d2239] hover:text-white"
                >
                  Todos os Jogos
                </button>
                {casinoLinks.slice(1).map((item) => (
                  <Link
                    key={item.label}
                    to={item.href}
                    className="block w-full rounded-md px-2 py-2 text-left text-base font-semibold text-slate-300 transition hover:bg-[#0d2239] hover:text-white"
          ...
          {showCrash ? (
            ...
            <section className="p-4 md:p-6">
              <div className="mb-5 flex items-center justify-between gap-3">
                ...
                <Button
                  className="border border-[#2a3e58] bg-transparent text-xs font-black uppercase hover:bg-[#15263b]"
                  onClick={() => setShowCasinoGames(false)}
                >
                  Voltar
                </Button>
              </div>
              ...
              <div className="mt-8">
                <h2 className="mb-5 text-3xl font-black uppercase tracking-wide">
                  Jogos do Tigre
                </h2>
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
                  {tigreGames.map((game) => (
            ...
            <>
                ...
                <div className="relative z-10 grid gap-4 p-6 md:grid-cols-2 md:p-8">
                  <div>
                    <h1 className="text-4xl font-black">
                      Bem-vindo ao NeonBet!
                    </h1>
                    {!initializing && isLoggedIn ? (
                      <>
                        <p className="mt-4 text-2xl font-semibold text-slate-100">
                          Aproveite os melhores jogos e ganhe grandes prêmios!
                        </p>
                        <Button
                          className="mt-8 border-0 bg-[#f02254] px-8 font-black uppercase hover:bg-[#ff2a60]"
                          onClick={() => setShowCasinoGames(true)}
                        >
                          Explorar Jogos
                        </Button>
                        <div className="mt-12 flex flex-wrap items-center gap-3 text-sm font-semibold text-slate-200">
                          <span>Mais de 1000 jogos disponíveis</span>
                  ...
                  <div className="flex justify-end">
                    <img
                      ...
                      className="hidden h-[350px] w-[500px] object-cover md:block"
                    />
                  </div>
                </div>
              </section>
              ...
              <section className="mt-5 grid gap-4 md:grid-cols-2">
                <article className="relative overflow-hidden rounded-sm border border-[#233750] bg-[#1b2b3d] p-6">
                  <img
                    src="https://images.unsplash.com/photo-1596838132731-3301c3ef77e4?w=900&h=450&fit=crop&q=80"
                    alt="Cassino"
                  ...
                  <div className="relative z-10 max-w-xs">
                    ...
                    <p className="mt-3 text-sm text-slate-200">
                      Aproveite nossa seleção exclusiva de slots, dealers ao
                      vivo e jogos originais.
                    </p>
                    <Button
                      className="mt-8 border-0 bg-[#f02254] px-8 font-black uppercase hover:bg-[#ff2a60]"
                      onClick={() => setShowCasinoGames(true)}
                ...
                <article className="relative overflow-hidden rounded-sm border border-[#233750] bg-[#1b2b3d] p-6">
                  <img
                    src="https://images.unsplash.com/photo-1521412644187-c49fa049e84d?w=900&h=450&fit=crop&q=80"
                    alt="Esportes"
                    className="absolute inset-0 h-full w-full object-cover opacity-25"
                  />
                  <div className="absolute inset-0 bg-gradient-to-r from-[#1b2b3d] via-[#1b2b3d]/95 to-[#1b2b3d]/50" />
                  <div className="relative z-10 max-w-xs">
                    ...
                    <p className="mt-3 text-sm text-slate-200">
                      Nossas apostas esportivas intuitivas sao feitas para
                      jogadores novos e experientes.
                    </p>
                    <Button className="mt-8 border-0 bg-[#f02254] px-8 font-black uppercase hover:bg-[#ff2a60]">
                      Ir para Esportes
                    </Button>
                  </div>
                </article>
              ...
              <section className="mt-6 rounded-sm border border-[#1d3047] bg-[#071a2f] py-5 text-center">
                ...
                <div className="mt-4 flex items-center justify-center gap-6">
                  <span className="text-5xl font-black tracking-widest text-slate-100">
                    pix
                  </span>
              ...
              <section className="mt-6 grid gap-4 md:grid-cols-2">
                {sponsorCards.map((card) => (
                  <article
                    key={card.title}
                    className="relative h-56 overflow-hidden rounded-sm border border-[#26384d]"
                  >
                    <img
                      src={card.image}
                      alt={card.title}
                    ...
                    <div className="absolute left-0 top-0 p-6">
                      <span className="rounded bg-white px-2 py-1 text-[10px] font-black uppercase text-[#0f2238]">
                        Patrocinio
                      </span>
                      <h3 className="mt-4 text-4xl font-black">{card.title}</h3>
                      <p className="mt-2 max-w-xs text-sm font-semibold text-slate-100">
                        {card.subtitle}
                      </p>
                    </div>
                  </article>
