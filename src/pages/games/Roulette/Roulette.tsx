...
export default function RouletteGame() {
  ...
  const startSpin = async () => {
    ...
    if (totalBet > 0) {
      if (totalWin > 0) {
        toast({
          title: `🎡 ${num} (${getColor(num).toUpperCase()})`,
          description: `Você ganhou R$ ${totalWin.toFixed(2)}!`,
        });
        addTransaction({
          type: "win",
          amount: totalWin,
          status: "approved",
          description: `Ganho na Roleta - número ${num}`,
        });
        addBet({
          game: "Roleta",
          betAmount: totalBet,
          result: totalWin / totalBet,
        ...
        toast({
          title: `🎡 ${num} (${getColor(num).toUpperCase()})`,
          description: `Perdeu R$ ${totalBet.toFixed(2)}`,
          variant: "destructive",
        });
        addBet({
          game: "Roleta",
  ...
  const addBetAction = (type: BetType["type"], value: number | string) => {
    ...
    toast({
      ...
      description: `R$ ${betAmount} em ${value}`,
    });
  };
                ...
                <div className="flex justify-center mb-6">
                  <div className="relative w-48 h-48">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div
                        className={`w-16 h-16 rounded-full border-4 border-primary/50 flex items-center justify-center text-2xl font-black ${phase === "result" ? (getColor(result!) === "red" ? "bg-destructive" : getColor(result!) === "black" ? "bg-casino-bg" : "bg-primary") : "bg-casino-card"}`}
                      >
                        {phase === "result" ? result : "?"}
                      </div>
                    </div>
                  ...
                  <div className="flex gap-1">
                    {[1, 5, 10, 25, 50].map((v) => (
                      <button
                        ...
                        className={`px-2 py-1 text-xs font-bold rounded border transition-colors ${betAmount === v ? "bg-primary border-primary text-white" : "bg-secondary border-border"}`}
                      >
                        {v}
                      </button>
                    ))}
                  ...
                  {[
                    {
                      ...
                      value: "red",
                      color:
                        "bg-destructive/20 hover:bg-destructive/40 border-destructive/30",
                    },
                    {
                      label: "Preto ⚫",
                      type: "black" as const,
                      value: "black",
                      color: "bg-secondary hover:bg-secondary/70 border-border",
                    },
                    {
                      label: "Verde 🟢 (0)",
                      ...
                      value: 0,
                      color:
                        "bg-neon-green/20 hover:bg-neon-green/30 border-neon-green/30",
                    },
                    {
                      label: "Par",
                      type: "even" as const,
                    ...
                    },
                    {
                      ...
                      type: "low" as const,
                      value: "1-18",
                      color: "bg-accent/20 hover:bg-accent/30 border-accent/30",
                    },
                    {
                      label: "19-36",
                      type: "high" as const,
              ...
              {bets.length > 0 && (
                <div className="card-casino rounded-xl border border-border p-3">
                  <p className="text-xs text-muted-foreground mb-2">
                    Apostas ativas: R${" "}
                    {bets.reduce((s, b) => s + b.amount, 0).toFixed(2)}
                  </p>
                  <div className="flex flex-wrap gap-1">
                    {bets.map((b, i) => (
                      <span
                        key={i}
                        className="px-2 py-0.5 bg-primary/20 text-primary rounded text-xs font-bold"
                      >
                        {b.value} · R${b.amount}
                      </span>
                    ))}
